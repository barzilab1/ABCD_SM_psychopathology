...
#################### other diagnosis #################### 
ksad_y_diagnosis_other = ksad_y[,grepl("src|inter|event|sex|_1(1|3)_9[1-4][0-9]_t",colnames(ksad_y))]
describe(ksad_y_diagnosis_other[ksad_y_diagnosis_other$eventname == "2_year_follow_up_y_arm_1",])

ksad_y_diagnosis_other = ksad_y_diagnosis_other[,colSums(is.na(ksad_y_diagnosis_other)) != nrow(ksad_y_diagnosis_other)]
ksad_y_diagnosis_other$Diagnosis_ed_y = apply(ksad_y_diagnosis_other[,grepl("ksads_13_.*_t", colnames(ksad_y_diagnosis_other))],1 ,function(x) {any(x == 1)*1})

write.csv(file = "outputs/ksad_y_ed.csv",x = ksad_y_diagnosis_other, row.names=F, na = "")
