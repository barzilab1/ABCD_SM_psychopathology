...
#################### other diagnosis #################### 
ksad_p_diagnosis_other = ksad_p[,grepl("src|inter|event|sex|_1(1|3)_9[1-4][0-9]_p",colnames(ksad_p))]
describe(ksad_p_diagnosis_other[ksad_p_diagnosis_other$eventname == "2_year_follow_up_y_arm_1",])


ksad_p_diagnosis_other$Diagnosis_ed_p = apply(ksad_p_diagnosis_other[,grepl("ksads_13_.*_p", colnames(ksad_p_diagnosis_other))],1 ,function(x) {any(x == 1)*1})
ksad_p_diagnosis_other$Diagnosis_ocd_p = apply(ksad_p_diagnosis_other[,grepl("ksads_11_.*_p", colnames(ksad_p_diagnosis_other))],1 ,function(x) {any(x == 1)*1})

write.csv(file = "outputs/ksad_p_other.csv",x = ksad_p_diagnosis_other, row.names=F, na = "")
