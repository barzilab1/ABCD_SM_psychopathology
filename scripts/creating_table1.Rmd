...
vars_main <- dataset_wide %>% 
  dplyr::select(matches(c("age_year_2", "sex_", "race_[wbanom]", "ethnicity", "income_2", "q3_e", "so_fac", "q[124]_e", "harm_2", "victim_2", "_perc$", "t_mean", "S[IA]_y_ever"))) %>% names()

vars_main_factor <- dataset_wide %>% 
  dplyr::select(matches(c("sex_", "race_[wbanom]", "ethnicity", "q[1-4]_e", "S[IA]_y_ever"))) %>% names()
```
