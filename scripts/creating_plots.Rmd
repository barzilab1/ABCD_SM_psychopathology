...
data_long <- read_csv("../outputs/dataset_SGM_long_mm.csv") 

# Add a cut off for reshist_state_so_factor at 10%
data_long <- data_long %>% 
  mutate(reshist_state_so_factor_at10 = 
           case_when(reshist_state_so_factor > (quantile(data_long$reshist_state_so_factor, probs = 0.9, na.rm = T) ) ~ 1,
                     reshist_state_so_factor <= (quantile(data_long$reshist_state_so_factor, probs = 0.9, na.rm = T) ) ~ 0,
                     TRUE ~ NA_real_))
```
...
data_wide <- data_wide %>% 
  ...
  mutate(bpm_y_scr_totalprob_t_mean = mean(c(bpm_y_scr_totalprob_t_1, bpm_y_scr_totalprob_t_2), na.rm = T),
         ...
         dim_yesno_q3_ever = case_when(dim_yesno_q3_1 == 1 | dim_yesno_q3_2 == 1 ~ 1,
                                       (dim_yesno_q3_1 == 0 | dim_yesno_q3_2 == 0) &
                                         # no need to check for 1, first case already checked that 
                                         (dim_yesno_q3_1 != 1 & dim_yesno_q3_2 != 1) ~ 0,
                                       TRUE ~ NA_real_),
         
         LGBT_inclusive_ever = case_when(LGBT_inclusive_1 == 1 | LGBT_inclusive_2 == 1 ~ 1,
                                         (LGBT_inclusive_1 == 0 | LGBT_inclusive_2 == 0) &
                                           (LGBT_inclusive_1 != 1 & LGBT_inclusive_2 != 1) ~ 0,
                                         TRUE ~ NA_real_))
